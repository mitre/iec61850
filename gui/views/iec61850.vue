...
onMounted(async () => {
    const response = await $api.get('/plugin/iec61850/data')
...
<template>
    <div>
        <h2>IEC 61850 Protocol Plugin</h2>
        <p>{{ description }}</p>
    </div>
    <hr>
    ...
    <p>
        ...
        <a :href="`/docs/plugins/${name.toLowerCase()}/${name.toLowerCase()}.html`" target="_blank">IEC 61850 Plugin Fieldmanual</a>.
    </p>
    ...
    <h3>Adversaries</h3>
    <p>There are no custom IEC 61850 adversaries at this time.</p>
</template>
